...
void CascTrackingStudy(std::string path)
{
    ...
    TH1D *hCascITStracksMom = new TH1D("v0_its_tracks_mom", ";#it{p}_{T} (GeV/#it{c}); Counts", 40, 1, 6);
    TH1D *hCascITStracksRadius = new TH1D("v0_its_tracks_radius", ";Radius (cm); Counts", 40, 0, 40);
    ...
    for (unsigned int i = 0; i < dirs.size(); i++)
    {
        ...
        for (int frame = 0; frame < treeITS->GetEntriesFast(); frame++)
        {
            ...
            for (unsigned int iCascVec = 0; iCascVec < cascVec->size(); iCascVec++)
            {
                ...
                for (unsigned int iITStrack = 0; iITStrack < ITStracks->size(); iITStrack++)
                {
                    ...
                    if (ITSref[0] == cascMCref[0] && ITSref[1] == cascMCref[1])
                    {
                        ...
                        hCascITStracksMom->Fill(casc.getPt());
                        hCascITStracksRadius->Fill(sqrt(casc.calcR2()));
                        ...
                        for (unsigned int iStTr = 0; iStTr < strangeTrackVec->size(); iStTr++)
                        {
                            ...
                            if (!(strangeTrack.mPartType == o2::dataformats::kStrkCascade))
                            {
                                continue;
                            }
                            ...
                            if (itsRef == iITStrack && cascRef == iCascVec)
                            {
            ...
            {
                LOG(info) << "+++++++++++++++++++++++++++++++++++++++++++++";
                LOG(info) << "Strange track " << iHyperVec << " of " << strangeTrackVec->size();
                ...
                bool isTrueCasc = true;
                bool isTrueITSmother = true;
                if (cascMCref[0] == -1 || cascMCref[1] == -1)
                    isTrueCasc = false;
                if (ITStrackRef[0] == -1 || ITStrackRef[1] == -1)
                    isTrueITSmother = false;

                if (ITStrackRef[0] == cascMCref[0] && ITStrackRef[1] == cascMCref[1] && isTrueCasc && isTrueITSmother)
                {
                    hXiStats->Fill(2);
                    ...
                    auto trackedR = sqrt(strangeTrack.mDecayVtx[0] * strangeTrack.mDecayVtx[0] + strangeTrack.mDecayVtx[1] * strangeTrack.mDecayVtx[1]);
                ...
                else
                {
                    ...
                    if (!isTrueCasc)
                        LOG(info) << "Fake cascade: " << cascMCref[0] << ", " << cascMCref[1];
                    if (!isTrueITSmother)
                        LOG(info) << "Fake ITS mother: " << ITStrackRef[0] << ", " << ITStrackRef[1];
                }
            }
        }
    }
    ...
    TH1D *hAlgoEffRad = (TH1D *)hTrackedXiRad->Clone("algo_eff");
    hAlgoEffRad->Divide(hCascITStracksRadius);
    auto cv8 = TCanvas("algo_efficiency_rad", "", 1000, 1000);
    hAlgoEffRad->GetXaxis()->SetTitle("Radius (cm)");
    hAlgoEffRad->GetYaxis()->SetTitle("Algorithm Efficiency");
    hAlgoEffRad->SetLineColor(kBlue);
    hAlgoEffRad->Draw();
    cv8.Write();

    TH1D *hAlgoEffMom = (TH1D *)hTrackedXiMom->Clone("algo_eff");
    hAlgoEffMom->Divide(hCascITStracksMom);
    auto cv9 = TCanvas("algo_efficiency_pt", "", 1000, 1000);
    hAlgoEffMom->GetYaxis()->SetTitle("Algorithm Efficiency");
    hAlgoEffMom->SetLineColor(kBlue);
    hAlgoEffMom->Draw();
    cv9.Write();
...
std::array<int, 2> matchCascToMC(const std::vector<std::vector<o2::MCTrack>> &mcTracksMatrix, std::map<std::string, std::vector<o2::MCCompLabel> *> &map, std::vector<V0> *v0vec, Cascade &casc, bool &isV0reco)
{
    ...
    if (!(std::abs(dau1MC.GetPdgCode()) == firstV0dauPDG && std::abs(dau2MC.GetPdgCode()) == secondV0dauPDG) && !(std::abs(dau1MC.GetPdgCode()) == secondV0dauPDG && std::abs(dau2MC.GetPdgCode()) == firstV0dauPDG))
        return motherVec;
    LOG(info) << "V0 daughters matched to MC";
    ...
    LOG(info) << "V0 daughters are secondary and have the same mother";
